...
async function configurePaidInvoicesPage() {
  ...
  const pageBody = [
    ...
    '      var invNames = invoices.map(function (i) { return i.name; });',
    '',
    '      // ── Step 2: Payment Entry References for these invoices ──────────',
    '      frappe.call({',
    '        method: "frappe.client.get_list",',
    '        args: {',
    ...
    '          filters: [',
    '            ["reference_doctype","=","Sales Invoice"],',
    '            ["reference_name","in",invNames]',
    '          ],',
    '          fields: ["parent","reference_name"],',
    '          limit_page_length: 200',
    '        },',
    '        callback: function (r2) {',
    '          // invToPE: { "ACC-SINV-…": "ACC-PAY-…" }',
    '          var invToPE = {};',
    '          if (r2.message) {',
    '            r2.message.forEach(function (ref) { invToPE[ref.reference_name] = ref.parent; });',
    '          }',
    '          var peNames = Object.values(invToPE).filter(Boolean);',
    '          if (!peNames.length) { renderInvoices(invoices, {}); return; }',
    '',
    '          // ── Step 3: reference_no (Stripe PI ID) from Payment Entries ─',
    '          frappe.call({',
    '            method: "frappe.client.get_list",',
    '            args: {',
    '              doctype: "Payment Entry",',
    '              filters: [["name","in",peNames]],',
    '              fields: ["name","reference_no"],',
    '              limit_page_length: 200',
    '            },',
    '            callback: function (r3) {',
    '              // peToStripe: { "ACC-PAY-…": "pi_3Abc…" }',
    '              var peToStripe = {};',
    '              if (r3.message) {',
    '                r3.message.forEach(function (pe) {',
    '                  if (pe.reference_no && pe.reference_no.startsWith("pi_")) {',
    '                    peToStripe[pe.name] = pe.reference_no;',
    '                  }',
    '                });',
    '              }',
    '              // invToStripe: { "ACC-SINV-…": "pi_3Abc…" }',
    '              var invToStripe = {};',
    '              Object.keys(invToPE).forEach(function (inv) {',
    '                var piId = peToStripe[invToPE[inv]];',
    '                if (piId) invToStripe[inv] = piId;',
    '              });',
    '              renderInvoices(invoices, invToStripe);',
    '            },',
    '            error: function () { renderInvoices(invoices, {}); }',
    '          });',
    '        },',
    '        error: function () { renderInvoices(invoices, {}); }',
    '      });',
    ...
    '  });',
    '',
    '  function renderInvoices(invoices, invToStripe) {',
    '    var rows = invoices.map(function (inv) {',
    '      var receiptBtn = "";',
    '      if (invToStripe[inv.name]) {',
    '        receiptBtn = \'<a href="https://dashboard.stripe.com/test/payments/\'',
    '          + encodeURIComponent(invToStripe[inv.name])',
    '          + \'" target="_blank" rel="noopener" class="btn btn-xs btn-default ml-2"\'',
    '          + \' style="font-size:11px;padding:2px 8px;">Stripe Receipt \u2197</a>\';',
    '      }',
    ...
    '        + \'<div class="col-sm-3 text-right"><strong>$\' + parseFloat(inv.grand_total).toFixed(2) + \'</strong>\' + receiptBtn + \'</div>\'',
    '        + \'</div></div>\';',
    '    }).join("");',
